...
<template>
  <header>
    <h1 class=" dark:text-zinc-300 text-3xl lg:text-4xl m-7 font-bold text-center">
      {{ title || '' }}
    </h1>
    <NuxtImg
      :src="image || ''"
      :alt="alt || ''"
      width="600"
      class="m-auto rounded-2xl shadow-lg h-32 md:h-72 w-4/6 md:w-4/5 content-center object-cover mb-6"
    />
    ...
    <div class="flex w-full justify-center text-xs md:text-base my-8">
      <div class="md:flex text-black dark:text-zinc-300 content-center gap-8 text-xs sm:text-sm">
        <div class="flex justify-center  md:items-center font-semibold">
          <LogoDate />
          <p>{{ date || '' }}</p>
        </div>
